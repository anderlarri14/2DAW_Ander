...
miAplicacion.controller('mainController', ["$scope", "$http", function($scope, $http) {
    ...
    $scope.cVentana = function (eleccion) {
        ...
        }else if ("filtrar" == eleccion) {
            $scope.zonaIkasleak = false;
            $scope.zonaCompra = false;
            $scope.zonaAdmin = false;
            $scope.zonaFiltrar = true;
    ...
    $scope.compra = function (){
        if ($scope.comboArticulos.Stock >= $scope.cantidad) {
            alert("Se ha realizado la compra");
            $scope.comboArticulos.Stock = $scope.comboArticulos.Stock - $scope.cantidad;

            $scope.listaCOMPRAS.push({
                identificador: $scope.comboArticulos.id,
                NombreProducto: $scope.comboArticulos.nombre,
                NombreUsuario: $scope.comboUsuarios.NombreUsuario,
                Cantidad: $scope.cantidad
            });
        } else {
            alert("No hay suficiente stock compra denegada");
